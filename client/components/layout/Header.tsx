...
> = {
  "/": {
    ...
    description:
      "Welcome back! Monitor your business performance and key metrics.",
    icon: LayoutDashboard,
  },
  "/customers": {
    title: "Customer & Service Management",
    description:
      "Integrated workflow: manage customers and create service orders seamlessly.",
    icon: Users,
  },
  "/orders": {
    title: "Order Workflow Management",
    description:
      "Complete order lifecycle with status tracking and customer integration.",
    icon: ClipboardList,
  },
  "/orders/active": {
    title: "Active Orders",
  ...
  "/inventory/tires": {
    ...
    description:
      "Track tire stock levels, purchases, and inventory management.",
    icon: Package,
  },
  "/invoices": {
  ...
  "/sales": {
    ...
    description:
      "Track sales performance, transactions, and revenue analytics.",
    icon: ShoppingCart,
  },
  "/sales/new": {
    title: "New Sale Transaction",
    description:
      "Create new sales transactions and process customer purchases.",
    icon: ShoppingCart,
  },
  "/sales/analytics": {
...
const getPageConfig = (pathname: string) => {
  ...
  if (
    ...
    !pathname.match(/\/(active|completed|job-cards)$/)
  ) {
    return {
      title: "Order Details",
...
export function Header({ onToggleSidebar, isCollapsed }: HeaderProps) {
  ...
  const { alerts, updateExpectedLeave, markLeft, activeVisits } =
    useVisitTracking();
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        {a.expectedLeaveAt && (
                          <p className="text-xs text-muted-foreground">
                            Expected:{" "}
                            {new Date(a.expectedLeaveAt).toLocaleTimeString()}
                          </p>
                        )}
                        <div className="flex gap-2 mt-2">
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              updateExpectedLeave(a.id, { addMinutes: 15 })
                            }
                          >
                            +15m
                          </Button>
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              updateExpectedLeave(a.id, { addMinutes: 30 })
                            }
                          >
                            +30m
                          </Button>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Expected:{" "}
                          {v.expectedLeaveAt
                            ? new Date(v.expectedLeaveAt).toLocaleTimeString()
                            : "-"}
                        </p>
                        <div className="flex gap-2 mt-2">
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              updateExpectedLeave(v.id, { addMinutes: 15 })
                            }
                          >
                            +15m
                          </Button>
                          <Button
                            ...
                            variant="outline"
                            onClick={() =>
                              updateExpectedLeave(v.id, { addMinutes: 30 })
                            }
                          >
                            +30m
                          </Button>
